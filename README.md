...
* At least 5 GB to store the block chain files stored using the [Pruning Options](https://github.com/kylemanna/docker-bitcoind/blob/master/docs/pruning.md)
...
1. Create a `bitcoind-data` volume to persist the bitcoind blockchain data, should exit immediately.  The `bitcoind-data` container will store the blockchain when the node container is recreated (software upgrade, reboot, etc):
        ...
        docker run --volume bitcoind-data:/bitcoin --name=bitcoind-node -d \
            -p 8333:8333 \
